        ...
        for (int j = -2; j <= 2; ++j)
        {
            for (int l = 2; l <= 2; ++l)
            {
                Eigen::Vector3d lattice_translation =
                    casmutils::xtal::coordinate::fractional_to_cartesian(Eigen::Vector3d(i, j, l), *fcc_lattice_ptr);
...
{
    auto already_within =
        casmutils::xtal::coordinate::fractional_to_cartesian(Eigen::Vector3d(0.25, 0.25, 0), *fcc_lattice_ptr);
    auto ws_within =
        casmutils::xtal::coordinate::fractional_to_cartesian(Eigen::Vector3d(0.25, 0.25, 0), *fcc_lattice_ptr);
    ...
    auto far_right =
        casmutils::xtal::coordinate::fractional_to_cartesian(Eigen::Vector3d(0.75, 0.25, 0), *fcc_lattice_ptr);
    auto far_far_right =
        casmutils::xtal::coordinate::fractional_to_cartesian(Eigen::Vector3d(1.75, 0.25, 0), *fcc_lattice_ptr);
    ws_within = casmutils::xtal::coordinate::fractional_to_cartesian(Eigen::Vector3d(-0.25, 0.25, 0), *fcc_lattice_ptr);
